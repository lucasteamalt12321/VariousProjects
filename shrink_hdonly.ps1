...
$root = 'C:\Users\admin\Documents\GD-shrink\gd2208_hdonly\assets'
...
Get-ChildItem -Path $root -Recurse -File -Filter '*-hd.png' |
    ForEach-Object { Resize-Png $_.FullName }

Get-ChildItem -Path $root -Recurse -File -Filter '*-hd.plist' |
    ForEach-Object {
        $path = $_.FullName
...
Get-ChildItem -Path $root -Recurse -File -Filter '*-hd.fnt' |
    ForEach-Object {
        $path = $_.FullName
        $content = [IO.File]::ReadAllText($path)
