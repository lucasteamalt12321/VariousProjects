...
$root = 'C:\Users\admin\Documents\GD-shrink\gd2208_unpacked\assets'

Add-Type -AssemblyName System.Drawing

function Scale-QuarterInt([string]$value) {
    $n = [double]$value
    $scaled = [math]::Round($n / 4.0, [System.MidpointRounding]::AwayFromZero)
    return ([int]$scaled).ToString()
}

function Scale-QuarterNumber([string]$value) {
    $n = [double]$value
    $scaled = [math]::Round($n / 4.0, [System.MidpointRounding]::AwayFromZero)
    return ([int]$scaled).ToString()
}
...
Get-ChildItem -Path $root -Recurse -File -Filter '*.plist' |
    ForEach-Object {
        ...
        $content = Replace-KeyedString $content @('textureRect', 'spriteSize', 'spriteOffset', 'spriteSourceSize', 'size', 'position')
...
Get-ChildItem -Path $root -Recurse -File -Filter '*.fnt' |
    ForEach-Object {
        $path = $_.FullName
        $content = [IO.File]::ReadAllText($path)
        $content = [regex]::Replace($content, '(?<name>size|lineHeight|base|scaleW|scaleH|width|height|xoffset|yoffset|xadvance|amount)=(?<value>-?\d+)', {
            param($m)
            return $m.Groups['name'].Value + '=' + (Scale-QuarterInt $m.Groups['value'].Value)
        })
        [IO.File]::WriteAllText($path, $content, [System.Text.UTF8Encoding]::new($false))
    }
